...
public:
  ...
  FaucetEmitter (b2World *world, const b2Vec2 &origin, b2Vec2 &startingVelocity, float size,
                 float emitRate, b2ParticleColor color) :
    m_startingVelocity(startingVelocity), m_origin(origin), m_size(size), m_emitRate(emitRate),
  ...
  void Step(float dt, uint32 flags)
  {
    ...
    while(m_counter > 1) {
      ...
      pd.position.Set(m_origin.x + (rand() % 100)/100.0f * m_size - m_size/2, m_origin.y);
      // Send it flying
...
public:
  ...
  Sandbox()
  {
    ...
    m_killfieldShape.SetAsBox(PLAYFIELD_RIGHT_EDGE - PLAYFIELD_LEFT_EDGE,1);
  ...
  //   A = a left-to-right ramp triangle (can't be \ or string formatting would be weird)
  //   r, g, b = colored faucets pointing down
  //   p = a pump block that rocks back and forth.  You can drag them yourself with your finger.
  //   C = a loose circle
  //   K = an ignored placeholder for a killfield to remove particles; entire bottom row is a
  //     killfield.
  void SetupMaze() {
    ...
    for (int i = 0; i < TILE_WIDTH; i++) {
      for (int j = 0; j < TILE_HEIGHT; j++) {
        ...
        b2Vec2 center = b2Vec2(PLAYFIELD_LEFT_EDGE + TILE_RADIUS * 2 * i + TILE_RADIUS,
  ...
  void AddPump(const b2Vec2 &center)
  {
    ...
    // Create a prismatic joint and connect to the ground, and have it slide along the x axis
  ...
  void AddFaucetEmitter(const b2Vec2 &center, b2ParticleColor &color)
  {
    ...
    b2Assert(m_faucetEmitterIndex < MAX_PUMPS);

    b2Vec2 startingVelocity = b2Vec2(0, PARTICLE_EXIT_Y_SPEED);

    m_emitters[m_faucetEmitterIndex] =
      new FaucetEmitter(m_world, center, startingVelocity, DEFAULT_EMITTER_SIZE,
                        DEFAULT_EMITTER_RATE, color);
  ...
  virtual void Step(Settings *settings)
  {
    ...
    for (int i = 0; i < m_pumpIndex; i++)
    {
      ...
      // Pumps can and will clog up if the pile of particles they're trying to push
      // is too heavy.  Increase PUMP_FORCE to make stronger pumps.
    ...
    m_debugDraw.DrawString(5, m_textLine, "Keys: (a) zero out (water), (q) powder");
...
private:
  ...
  // Pumps and emitters
  b2Body* m_pumps[MAX_PUMPS];
  FaucetEmitter *m_emitters[MAX_EMITTERS];
