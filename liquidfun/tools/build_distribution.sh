...
# This is the old Box2D Readme.txt, LiquidFun version is in
...
# Brand files with version information by replacing LIQUIDFUN_VERSION with
# "${1}".
...
"
  ...
  (
    ...
    for f in $(echo "${files_to_brand}" | grep -v '^#'); do
      sed -i 's/LIQUIDFUN_VERSION/'"${version}"'/g' "${f}"
    done
  )
}
